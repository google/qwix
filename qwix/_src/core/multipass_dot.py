...
def residual_decompose(
    x: jax.Array,
    how: qarray.HowToQuantize | Sequence[qarray.HowToQuantize],
    n_passes: int = 2,
) -> tuple[qarray.QArray, ...]:
  ...
  block-level reduction passes. If how is a sequence of HowToQuantize rules,
  each pass is sequentially quantized with its corresponding rule.

  Args:
    x: Unquantized input jax.Array.
    how: HowToQuantize configuration or sequence of configurations.
...
def multipass_dot_general(
  ...
  if mode == 'lhs_high_precision':
    ...
    eff_tile_size = 256 if tile_size is None else tile_size
    ...
    return asymmetric_int_dot_general(
      ...
      'hybrid_fp8_fp4',
      'hybrid_fp8_fp4_full_cross',
      'hybrid_fp8_fp4_lhs_high_precision',
      'hybrid_fp8_fp4_rhs_high_precision',
  ):
    hybrid_mode = {
        'hybrid_fp8_fp4': 'triangular',
        'hybrid_fp8_fp4_full_cross': 'full_cross',
        'hybrid_fp8_fp4_lhs_high_precision': 'lhs_high_precision',
        'hybrid_fp8_fp4_rhs_high_precision': 'rhs_high_precision',
    }[mode]
    return hybrid_fp8_fp4_multipass_dot_general(
        lhs,
        rhs,
        dimension_numbers=dimension_numbers,
        mode=hybrid_mode,
    ...
    raise ValueError(
        ...
        " 'int4_int8', 'hybrid_fp8_fp4', 'hybrid_fp8_fp4_full_cross',"
        " 'hybrid_fp8_fp4_lhs_high_precision',"
        " 'hybrid_fp8_fp4_rhs_high_precision'."
    )
...
def hybrid_fp8_fp4_multipass_dot_general(
    lhs: jax.Array,
    rhs: jax.Array,
    dimension_numbers: jax.lax.DotDimensionNumbers = (((1,), (0,)), ((), ())),
    mode: MultiPassMode = 'triangular',
    *,
    lhs_fp8_qtype: jax.typing.DTypeLike = jnp.float8_e4m3fn,
    rhs_fp8_qtype: jax.typing.DTypeLike = jnp.float8_e4m3fn,
    lhs_fp4_qtype: jax.typing.DTypeLike = jnp.float4_e2m1fn,
    rhs_fp4_qtype: jax.typing.DTypeLike = jnp.float4_e2m1fn,
    tile_size: int | Sequence[int | None] | None = None,
    lhs_fp8_tile_size: int | None = 256,
    rhs_fp8_tile_size: int | None = 256,
    lhs_fp4_tile_size: int | None = 256,
    rhs_fp4_tile_size: int | None = 256,
    preferred_element_type: jax.typing.DTypeLike | None = None,
    **kwargs: Any,
) -> jax.Array:
  """Hybrid multi-pass GEMM: FP8 for Pass 1, FP4 for Pass 2."""
  if tile_size is not None:
    if isinstance(tile_size, int):
      lhs_fp8_tile_size = rhs_fp8_tile_size = tile_size
      lhs_fp4_tile_size = rhs_fp4_tile_size = tile_size
    elif isinstance(tile_size, (list, tuple)):
      lhs_fp8_tile_size = rhs_fp8_tile_size = tile_size[0]
      lhs_fp4_tile_size = rhs_fp4_tile_size = tile_size[1]

  if lhs_fp4_qtype == 'mxfp4' and lhs_fp4_tile_size != 32:
    lhs_fp4_qtype = jnp.float4_e2m1fn
  if rhs_fp4_qtype == 'mxfp4' and rhs_fp4_tile_size != 32:
    rhs_fp4_qtype = jnp.float4_e2m1fn
  if lhs_fp8_qtype in ('mxfp8', 'mxfp8_16') and lhs_fp8_tile_size not in (
      16,
      32,
  ):
    lhs_fp8_qtype = jnp.float8_e4m3fn
  if rhs_fp8_qtype in ('mxfp8', 'mxfp8_16') and rhs_fp8_tile_size not in (
      16,
      32,
  ):
    rhs_fp8_qtype = jnp.float8_e4m3fn

  how_l1 = get_how_to_quantize(
      dimension_numbers=dimension_numbers,
      ndims=(lhs.ndim, rhs.ndim),
      for_lhs=True,
      qtype=lhs_fp8_qtype,
  ...
  a_passes = residual_decompose(lhs, [how_l1, how_l2])
  b_passes = residual_decompose(rhs, [how_r1, how_r2])
