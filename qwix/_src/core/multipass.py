...
def residual_decompose_hybrid(
    x: jax.Array,
    hows: Sequence[qarray.HowToQuantize],
) -> tuple[qarray.QArray, ...]:
  """Multi-pass residual decomposition with heterogeneous quantization rules.

  Decomposes input tensor x into sequential residual passes using a distinct
  HowToQuantize rule for each pass (e.g. Pass 1: mxfp8_16, Pass 2: mxfp4).

  For each pass i with rule hows[i]:
    q_i = quantize(r_i, hows[i])
    x_i = dequantize(q_i)
    r_{i+1} = r_i - x_i

  Args:
    x: Input continuous array to decompose.
    hows: Sequence of HowToQuantize rules, one for each pass.

  Returns:
    Tuple of quantized QArray components (q_0, q_1, ...).
  """
  passes = []
  current = x
  for how in hows:
...
def hybrid_fp8_fp4_multipass_dot(
    lhs: jax.Array,
    rhs: jax.Array,
    dimension_numbers: jax.lax.DotDimensionNumbers = (((1,), (0,)), ((), ())),
    mode: MultiPassMode = 'triangular',
    *,
    lhs_fp8_tile_size: int = 16,
    rhs_fp8_tile_size: int = 16,
    lhs_fp4_tile_size: int = 32,
    rhs_fp4_tile_size: int = 32,
    preferred_element_type: jax.typing.DTypeLike | None = None,
) -> jax.Array:
  """Hybrid multi-pass GEMM: mxfp8_16 for Pass 1, mxfp4 for Pass 2.

  Evaluates cross-precision multi-pass matrix multiplication:
  - Pass 1 (Coarse): Quantizes input to mxfp8_16 (block size 16), capturing
    macroscopic dynamic range.
  - Pass 2 (Fine): Quantizes residual to mxfp4 (block size 32), recovering fine
    gradient and activation details.

  Execution modes:
  - 'triangular' (3 passes): A_0 B_0 + A_0 B_1 + A_1 B_0 (drops second-order
    cross-residual A_1 B_1, achieving >42 dB SQNR).
  - 'full_cross' (4 passes): A_0 B_0 + A_0 B_1 + A_1 B_0 + A_1 B_1 (evaluates
    all 4 cross-products, achieving >44 dB SQNR).
  - 'lhs_high_precision' (2 passes): A_0 B_0 + A_1 B_0 (1-sided: activations 2
    passes, weights 1 pass).
  - 'rhs_high_precision' (2 passes): A_0 B_0 + A_0 B_1 (1-sided: weights 2
    passes, activations 1 pass).
  - 'reconstructed': (A_0 + A_1) @ (B_0 + B_1).

  Args:
    lhs: Left operand array.
    rhs: Right operand array.
    dimension_numbers: Standard JAX dot_general dimension numbers.
    mode: Multi-pass combination mode.
    lhs_fp8_tile_size: Tile size for lhs Pass 1 (mxfp8_16, default 16).
    rhs_fp8_tile_size: Tile size for rhs Pass 1 (mxfp8_16, default 16).
    lhs_fp4_tile_size: Tile size for lhs Pass 2 (mxfp4, default 32).
    rhs_fp4_tile_size: Tile size for rhs Pass 2 (mxfp4, default 32).
    preferred_element_type: Output array element type (default
      float32/bfloat16).
  ...
  rhs_how_p2 = dot_general.get_how_to_quantize(
      ...
      for_lhs=False,
      qtype='mxfp4',
      tile_size=rhs_fp4_tile_size,
  )

  # pylint: disable=unbalanced-tuple-unpacking
  if mode == 'lhs_high_precision':
    a0_q, a1_q = residual_decompose_hybrid(lhs, (lhs_how_p1, lhs_how_p2))
    b0_q = qarray.quantize(rhs, rhs_how_p1)
    a0 = qarray.dequantize(a0_q)
    a1 = qarray.dequantize(a1_q)
    b0 = qarray.dequantize(b0_q)
    c00 = lax.dot_general(
        a0, b0, dimension_numbers, preferred_element_type=jnp.float32
    )
    c10 = lax.dot_general(
        a1, b0, dimension_numbers, preferred_element_type=jnp.float32
    )
    res = c00 + c10
  elif mode == 'rhs_high_precision':
    a0_q = qarray.quantize(lhs, lhs_how_p1)
    b0_q, b1_q = residual_decompose_hybrid(rhs, (rhs_how_p1, rhs_how_p2))
    a0 = qarray.dequantize(a0_q)
    b0 = qarray.dequantize(b0_q)
    ...
    c01 = lax.dot_general(
        a0, b1, dimension_numbers, preferred_element_type=jnp.float32
    )
    res = c00 + c01
  else:
    a0_q, a1_q = residual_decompose_hybrid(lhs, (lhs_how_p1, lhs_how_p2))
    ...
    if mode == 'reconstructed':
      ...
      if mode == 'triangular':
        ...
        c11 = lax.dot_general(
            a1, b1, dimension_numbers, preferred_element_type=jnp.float32
        )
        res = c00 + c01 + c10 + c11
      else:
