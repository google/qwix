...
Naive, etc.) are included for reference only to model hardware feasibility on
native INT4 units. For software emulation experiments using BF16, you should
simply use regular single-pass mxint8.
...
def mxint8_multipass_dot(
    ...
    dimension_numbers: jax.lax.DotDimensionNumbers = (((1,), (0,)), ((), ())),
    *,
    method: Literal['karatsuba', 'naive'] = 'karatsuba',
    tile_size: int = 32,
    preferred_element_type: jax.typing.DTypeLike | None = None,
) -> jax.Array:
  """Executes microscaled mxint8 GEMM emulated via native INT4 building blocks.

  Computes per-block exact INT8 products using either Karatsuba (3 INT4 passes)
  or Naive (4 INT4 passes), scaling each block product by (scale_lhs *
  scale_rhs).
  ...
  Args:
    ...
    dimension_numbers: Standard JAX dot_general dimension specification.
    method: 'karatsuba' (3 passes) or 'naive' (4 passes).
  ...
  dot_fn = (
      karatsuba_signed_int8_dot
      if method == 'karatsuba'
      else naive_signed_int8_dot
  )
