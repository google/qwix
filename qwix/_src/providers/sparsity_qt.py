...
from qwix._src.core import sparsity_core
...
class SparsityModule(nn.Module):
  ...
  def _maybe_update_mask(
    ...
    def mask_update(w: jax.Array, mask_val: jax.Array) -> jax.Array:  # pylint: disable=unused-argument
      if self.sparsity_rule is None:
        return mask_val
      return sparsity_core.get_pruning_n_m_mask(
          w,
          n=self.sparsity_rule.weight_sparsity_n,
          m=self.sparsity_rule.weight_sparsity_m,
          order=self.sparsity_rule.weight_sparsity_order,
          offset=self.sparsity_rule.weight_sparsity_offset,
          step=step,
          start_step=self.sparsity_rule.weight_sparsity_start_step,
          mask_dtype=mask_val.dtype,
      )
  ...
  def __call__(
    ...
    if self.sparsity_config.activation_sparsity_m != 0:
      input_mask = sparsity_core.get_pruning_n_m_mask(
          inputs,
          n=self.sparsity_config.activation_sparsity_n,
          m=self.sparsity_config.activation_sparsity_m,
          order=self.sparsity_config.activation_sparsity_order,
          offset=self.sparsity_config.activation_sparsity_offset,
      )
      inputs = jnp.where(
          input_mask, inputs, jnp.zeros(inputs.shape, inputs.dtype)
      )
    if self.sparsity_config.weight_sparsity_m != 0:
