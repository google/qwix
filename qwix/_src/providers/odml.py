    ...
    if len(args) == 3:
      ...
      if (
        ...
        if (
          ...
          # Forward Propagation: If lhs already has a quantized tracer cached
          # (FQ_ARRAY) from a sibling branch, reshape it to match args[1] shape
          # and cache it on args[1].
          ...
          aux_data.set(
              args[2], odml_ops.AuxDataKey.FLATTENED_EINSUM_WEIGHT, True
          )
          out = _einsum('ab,bc->ac', args[1], args[2], **kwargs)

          # Backward Propagation: If lhs was not already quantized (meaning we
          # are the first sibling to run), the _einsum call fake-quantized
          # args[1] and cached it as FQ_ARRAY. We reshape it back to lhs shape
          # and cache it on lhs so sibling branches can reuse it.
          if fq_lhs is None:
  ...
  def _fake_quant(
    ...
    with jax.ensure_compile_time_eval():
      ...
      if weight_name is not None:  # Weights.
        ...
        if aux_data.get(
            array, odml_ops.AuxDataKey.FLATTENED_EINSUM_WEIGHT, False
        ):
