...
class UfuncCall(QuantizedOp):
  ...
  # LiteRT classifies MINIMUM and MAXIMUM as same-scale ops: both operands and
  # the result must carry identical quantization parameters. Independently
  # calibrated input scales therefore make the converter fail with a "violate
  # the same scale constraint" error. Treat them the way the op table treats
  # clip and pad -- leave the inputs alone and let the scale come from the
  # output, which is sound because the result is always one of the inputs.
  _SAME_SCALE_OPS = frozenset({'maximum', 'minimum'})
  ...
  def _fake_quant_inputs(
    ...
    """Fake quantize the inputs of the op."""
    if self._op_name in self._SAME_SCALE_OPS:
      return args
    if (
