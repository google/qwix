...
def _apply_interceptors(
  ...
  # Op ids are positional within a module body, so each invocation of a module
  # has to start counting from zero. This is registered outside the op
  # interceptors so it covers the whole call.
  @functools.wraps(method)
  def method_with_per_invocation_op_ids(*args, **kwargs):
    if not should_intercept():
      return method(*args, **kwargs)
    with nn.intercept_methods(provider.reset_op_ids_on_module_entry):
      return method(*args, **kwargs)

  return method_with_per_invocation_op_ids
