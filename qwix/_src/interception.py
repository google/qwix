...
def wrap_func_intercepted(
  ...
  * To support patching PjitFunction object, we disable JIT if any of those
  ...
  def wrapper(*args, **kwargs):
    ...
    if (
        interception_manager.is_installed(interceptor_id)
        or not should_intercept()
    ):
    ...
    # Preprocess the interceptor for some JAX specific rewrites.
    for name in list(interceptor):
      ...
      if isinstance(original_fn, jax._src.lib._jax.PjitFunction):  # pylint: disable=protected-access
        interceptor[name + "._fun"] = interceptor.pop(name)
        need_to_disable_jit = True
        name = name + "._fun"
        original_fn = original_fn._fun  # pylint: disable=protected-access

      # Rewrite functions to code objects if they don't have freevars.
      if (
    ...
    interception_manager.install(interceptor_id, interceptor)
    ...
    try:
      ...
      interception_manager.uninstall(interceptor_id)
...
class _InterceptionManager:
  ...
  * Nested interception: it's possible to install different interceptors in a
  ...
  be a singleton and be protected by a lock.
  """
  ...
  def __init__(self):
    ...
    # A mapping from intercepted function names to the original functions.
  ...
  def is_installed(self, interceptor_id: int) -> bool:
    """Returns whether the interceptor is installed for the current thread."""
    this_thread = threading.get_ident()
    with self._lock:
      return (this_thread, interceptor_id) in self._intercepted_threads

  def install(self, interceptor_id: int, interceptor: Interceptor):
    """Installs the interceptor for the current thread."""
    this_thread = threading.get_ident()
    with self._lock:
      if (this_thread, interceptor_id) in self._intercepted_threads:
        raise ValueError(f"{interceptor_id} already installed {this_thread}.")
      self._intercepted_threads[(this_thread, interceptor_id)] = True
      # Add the interceptor to the list only once.
      ...
      try:
        for name in interceptor:
          self._try_register_interception(name)
        ...
        for name in registered:
          self._try_unregister_interception(name)
        raise e

  def uninstall(self, interceptor_id: int):
    """Uninstalls the interceptor for the current thread."""
    this_thread = threading.get_ident()
    with self._lock:
      # The current thread must already be intercepted.
      if (this_thread, interceptor_id) not in self._intercepted_threads:
        raise ValueError(f"{interceptor_id} not installed for {this_thread}.")
  ...
  def _try_register_interception(self, name: str):
    """Tries to register the interception for one name.

    * Do nothing if the interception is already registered.
    * Raise ValueError if we accidentally register different aliases for the
      same function.
    ...
    if name_parts[-1] == "__code__":  # special handling for code objects.
      ...
      setattr(obj, name_parts[-1], _fn_to_code(replacing_fn))
    else:
      original_fn = getattr(obj, name_parts[-1])
      # Check if we accidentally register different aliases for the same object.
      ...
      setattr(obj, name_parts[-1], replacing_fn)

  def _try_unregister_interception(self, name: str):
    """Tries to unregister the interception for one name.

    * Raise KeyError if the interception is not registered.
    * Do nothing if there is any other interceptor still needing the same
    ...
    for attr in name_parts[1:-1]:
      obj = getattr(obj, attr)
    if name_parts[-1] == "__code__":
      # Special handling for code objects.
      setattr(obj, name_parts[-1], self._original_fns.pop(name).__code__)
    else:
      setattr(obj, name_parts[-1], self._original_fns.pop(name))
