...
def _preprocess_interceptor(
  ...
  """Preprocesses the interceptor by rewriting keys and computing a stable ID.
  ...
  - **JAX `PjitFunction` (when `disable_jit=True`):** Rewritten to use `._fun`
    (and eventually `._fun.__code__`) to bypass JAX's C++ dispatch and target
    the inner Python bytecode.
  - **Everything Else:** Keys remain as-is, which tells the manager to use
    standard module-level attribute monkey-patching.

  Args:
    interceptor_dict: A mapping from function names to their replacement
      handlers.
    disable_jit: Whether JIT is disabled. Affects how `PjitFunction`s are
      handled.

  Returns:
    A tuple of (preprocessed_interceptor_dict, stable_interceptor_id).
  """
