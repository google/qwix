...
def pallas_call(
  ...
  def wrapper(*args):
    ...
    return pl.pallas_call(
        lambda *args: kernel(
            *args[:input_start],
            *restore_fn(args[input_start:input_end]),
            *args[input_end:],
        ),
        out_shape,
...
def _update_block_specs_for_qarray(block_specs: Any, args: Any) -> Any:
  ...
  def _quantize_in_spec(spec: pl.BlockSpec, arg):
    ...
    scale_index_map = lambda *args: tuple(
        i if s > 1 else 0
        for i, s in zip(spec.index_map(*args), arg.scale.shape)
    )
  ...
  return jax.tree.map(_quantize_in_spec, block_specs, args)
  ...
  for i, (spec, arg) in enumerate(zip(flatten_block_specs, flatten_args)):
    ...
    if _can_fit_tpu_requirements(block_shape_t, _reorder(arg.shape, transpose)):
      ...
      index_map_t = functools.partial(
          lambda spec, t, *args: _reorder(spec.index_map(*args), t),
          spec,
          transpose,
      )
    ...
    index_map_t = functools.partial(
        lambda spec, *args: spec.index_map(*args) + (0, 0), spec
    )
    flatten_block_specs[i] = dataclasses.replace(
        spec, block_shape=block_shape_t, index_map=index_map_t
    )
