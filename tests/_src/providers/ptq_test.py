...
class PtqTest(parameterized.TestCase):
  ...
  def _get_quantized_asarray_model(self):
    ...
    model = AsArrayModule()
    model = qwix_model.quantize_model(model, provider, jnp.ones((2, 2)))
    return model

  def test_asarray_interception_with_aux(self):
    """asarray should not dequantize WithAux."""
  ...
  def test_asarray_interception_qarray(self):
    """asarray should not dequantize QArray."""
    model = self._get_quantized_asarray_model()

    # Case 2: QArray
    qa = ptq.qarray.QArray(jnp.ones((2, 2), jnp.int8), jnp.ones((1, 1)))
    self.assertEqual(jax.tree.map(id, model(qa)), jax.tree.map(id, qa))

  def test_asarray_interception_nnx_state(self):
    """asarray should reconstruct QArray from nnx.State and fallback for others."""
    model = self._get_quantized_asarray_model()

    # Case 3: regular nnx.State (unboxing)
    state = nnx.State({"value": jnp.ones(5)})
    self.assertIsInstance(model(state.value), jax.Array)
  ...
  def test_asarray_interception_linen_module(self):
    """asarray should gracefully fallback for Linen Modules."""
    # Case 5: End-to-end Linen Module
    q_rules = [qconfig.QuantizationRule(weight_qtype=jnp.int8)]
    provider = ptq.PtqProvider(q_rules)

    class LinenAsArrayModule(nn.Module):
    ...
    linen_model = LinenAsArrayModule()
    prng = jax.random.PRNGKey(0)
    vars_ = linen_model.init(prng, jnp.ones((2, 2)))

    q_vars = ptq.quantize_params(vars_, jax.eval_shape(lambda: vars_))
    q_linen_model = qwix_model.quantize_model(linen_model, provider)

    res_linen = q_linen_model.apply(q_vars, jnp.ones((2, 2)))
    self.assertEqual(res_linen.shape, (2, 2))

  def test_asarray_with_aux_qarray(self):
