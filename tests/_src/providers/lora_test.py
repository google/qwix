...
class LoraTest(parameterized.TestCase):
  ...
  def test_lora_einsum_nnx_sharded(
    ...
    if apply_sharding_to_base_model:
      ...
      self.assertEqual(einsum.kernel.sharding_metadata, ("fsdp", "tp", None))
      self.assertEqual(einsum.kernel.value.sharding.spec, ("fsdp", "tp", None))
      self.assertEqual(einsum.bias.sharding_metadata, ("tp", None))
    ...
    self.assertEqual(lora_state.kernel_lora_a.sharding_metadata, ("fsdp", None))
    self.assertEqual(lora_state.kernel_lora_b.value.shape, (3, 8, 10))
    self.assertEqual(
        lora_state.kernel_lora_b.sharding_metadata, (None, "tp", None)
    )
    if weight_qtype is None:  # unquantized
      self.assertEqual(
          base_state.kernel.sharding_metadata, ("fsdp", "tp", None)
      )
      self.assertEqual(
      ...
      self.assertEqual(
          base_state.kernel.array.qvalue.sharding_metadata, ("fsdp", "tp", None)
      )
      self.assertEqual(base_state.kernel.array.scale.value.shape, (4, 8, 10))
      self.assertEqual(
          base_state.kernel.array.scale.sharding_metadata, ("fsdp", "tp", None)
      )
  ...
  def test_lora_conv_nnx(self, qtype):
    ...
    self.assertEqual(conv.kernel.sharding_metadata, (None, None, "in", "out"))
    ...
    self.assertEqual(lora_a.sharding_metadata, (None, None, "in", None))
    ...
    self.assertEqual(lora_b.sharding_metadata, (None, "out"))
