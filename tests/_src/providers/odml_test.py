...
class OdmlTest(parameterized.TestCase):
    ...
    class EinsumModel(nn.Module):
      ...
      def __call__(self, x):
        if not transform_weight:
          ...
          w = self.param('kernel', nn.initializers.lecun_normal(), weight_shape)
          w = jnp.flip(w, axis=0)
  ...
  def test_per_tensor_activation_scale_applies_at_any_rank(self):
    """Static activation scales are per-tensor, so their rank is incidental."""

    # A model is calibrated on training batches but converted against whatever
    # shapes the serving signature declares, and the two need not have the same
    # rank. The stored statistic is a single element either way, so re-ranking
    # it is numerically a no-op -- but the divide in
    # `quantize_with_scale_zero_point` asserts equal ranks rather than relying
    # on broadcasting.
    rules = [
        qconfig.QuantizationRule(
            module_path='.*', weight_qtype=jnp.int8, act_qtype=jnp.int8
    ...
    model = nn.Dense(8, use_bias=False)

    rank3_input = jnp.linspace(-1.0, 1.0, 2 * 3 * 16).reshape(2, 3, 16)
    qat_model = qwix_model.quantize_model(model, odml.OdmlQatProvider(rules))
    qat_vars = qat_model.init(jax.random.key(0), rank3_input)
    _, new_vars = qat_model.apply(qat_vars, rank3_input, mutable=True)
    qat_vars.update(new_vars)

    conversion_model = qwix_model.quantize_model(
        model,
        odml.OdmlConversionProvider(
            rules, qat_vars['params'], qat_vars['quant_stats']
        ),
    )
    rank2_input = jnp.linspace(-1.0, 1.0, 16).reshape(1, 16)
    conversion_res = conversion_model.apply(qat_vars, rank2_input)

    self.assertEqual(conversion_res.shape, (1, 8))
    self.assertTrue(jnp.all(jnp.isfinite(conversion_res)))

  def test_uncalibrated_op_falls_back_to_float(self):
    """An op QAT never saw stays in float instead of aborting the conversion."""

    class MaybeScaled(nn.Module):
      # Inference-only code is the common case: an exported signature can
      # reach arithmetic the training graph never executes, so no statistic
      # was ever collected for it.
      add_uncalibrated_op: bool = False

      @nn.compact
      def __call__(self, x):
        w = self.param('w', nn.initializers.normal(), (16, 8))
        y = jnp.dot(x, w)
        if self.add_uncalibrated_op:
          # Deliberately an identity. The model's *output* is itself a
          # static-range quantization point, calibrated during QAT, so an op
          # that changed the output range would be clamped to that range and
          # the comparison below would measure saturation rather than this
          # fix.
          y = y * jnp.asarray(1.0)
        return y
    ...
    qat_model = qwix_model.quantize_model(
        MaybeScaled(), odml.OdmlQatProvider(rules)
    )
    qat_vars = qat_model.init(jax.random.key(0), model_input)
    _, new_vars = qat_model.apply(qat_vars, model_input, mutable=True)
    qat_vars.update(new_vars)

    def _convert(add_uncalibrated_op):
      # A fresh provider per conversion: providers carry per-run state.
      conversion_model = qwix_model.quantize_model(
          MaybeScaled(add_uncalibrated_op=add_uncalibrated_op),
          odml.OdmlConversionProvider(
              rules, qat_vars['params'], qat_vars['quant_stats']
          ),
      )
      return conversion_model.apply(qat_vars, model_input)

    baseline = _convert(add_uncalibrated_op=False)

    # Convert the variant with the extra op, reusing statistics that never
    # covered it. The QAT model did not simulate quantization there either, so
    # leaving it in float is what reproduces the trained network.
    with self.assertLogs(level='WARNING') as logs:
      with_extra_op = _convert(add_uncalibrated_op=True)

    self.assertTrue(
        any('No quant_stats for' in line for line in logs.output), logs.output
    )
    # Compare the two conversions rather than comparing against the QAT run:
    # QAT derives its scales in-graph from the running statistics while
    # conversion bakes in the final ones, so the two drift for reasons that
    # have nothing to do with this fix. Between the two conversions the only
    # difference is the identity op, and leaving it in float means it cannot
    # perturb the result at all.
    self.assertTrue(
        jnp.array_equal(with_extra_op, baseline),
        f'the uncalibrated op perturbed the result: {with_extra_op} vs'
        f' {baseline}',
    )
  ...
  def test_linen_einsum_activation_only_conversion(self):
    """Tests activation-only quantization needs no RHS scale flattening."""
    rules = [
        qconfig.QuantizationRule(
            module_path='.*',
            act_qtype=jnp.int8,
    ...
    # The RHS-only rewrite falls back without weight quantization, so the
  ...
  def test_linen_einsum_dynamic_activation_conversion(self):
    """Tests RHS scale flattening leaves dynamic activations unchanged."""
    rules = [
        qconfig.QuantizationRule(
            module_path='.*',
            weight_qtype=jnp.int8,
  ...
  def test_linen_einsum_disable_per_channel_weight_conversion(self):
    """Tests per-tensor weights need no RHS scale flattening."""
    rules = [
        qconfig.QuantizationRule(
            module_path='.*',
            weight_qtype=jnp.int8,
  ...
  def test_flatten_einsum_interceptor_and_gating(self):
    """Tests the einsum handler is always installed and gates its rewrite."""
    weight_rule = qconfig.QuantizationRule(
        module_path='.*', op_names=('einsum',), weight_qtype=jnp.int8
    )
    unrelated_weight_rule = qconfig.QuantizationRule(
        module_path='.*', op_names=('dot_general',), weight_qtype=jnp.int8
    )
    activation_rule = qconfig.QuantizationRule(
        module_path='.*', op_names=('einsum',), act_qtype=jnp.int8
    )
    testcases = (
        ('per_channel_weight', [weight_rule], {}, 'BTD,DN->BTN'),
        (
            'activation_only_with_unrelated_weight_rule',
            [unrelated_weight_rule, activation_rule],
            {},
            'BTD,NDH->BTNH',
        ),
        (
            'unmatched_with_unrelated_weight_rule',
            [unrelated_weight_rule],
            {},
            'BTD,NDH->BTNH',
        ),
        (
    ...
    for name, rules, provider_kwargs, expected_einsum in testcases:
      with self.subTest(name):
        ...
        is_flatten_handler = (
            ...
            == provider._flatten_einsum  # pylint: disable=protected-access
        )
        self.assertTrue(is_flatten_handler)

        mock_einsum = mock.Mock(side_effect=jnp.einsum)
        mock_einsum.disable_per_channel_weights = provider_kwargs.get(
            'disable_per_channel_weights', False
        )
        ...
        self.assertEqual(mock_einsum.call_args[0][0], expected_einsum)
  ...
  def test_linen_einsum_general_flattened_rhs_conversion(
  ...
  def test_linen_transformed_einsum_weight_conversion(self):
    """Tests transformed RHS weight views raise when per-channel weights are requested."""
    rules = [
        qconfig.QuantizationRule(
            module_path='.*',
            weight_qtype=jnp.int8,
    ...
    with self.assertRaisesRegex(
        ValueError, 'requires multiple quantization dimensions'
    ):
      self._run_linen_einsum_conversion(rules, transform_weight=True)

    _, qat_res, conversion_res = self._run_linen_einsum_conversion(
        rules,
        provider_kwargs={'disable_per_channel_weights': True},
        transform_weight=True,
    )
    self.assertTrue(jnp.allclose(qat_res, conversion_res))

  def test_linen_transformed_einsum_activation_only_conversion(self):
    """Tests transformed RHS weight views succeed for activation-only ops."""
    rules = [
        qconfig.QuantizationRule(
    ...
    _, qat_res, conversion_res = self._run_linen_einsum_conversion(
        rules, transform_weight=True
    )
    self.assertTrue(jnp.allclose(qat_res, conversion_res))

  def test_linen_einsum_flatten_requires_matching_static_weight_shape(self):
    """Tests a lifted static parameter axis keeps einsum on the old path."""
    rules = [qconfig.QuantizationRule(module_path='.*', weight_qtype=jnp.int8)]
    lhs = jnp.ones((2, 3, 5), dtype=jnp.float32)
    rhs = jnp.ones((4, 5, 7), dtype=jnp.float32)
    lifted_static_rhs = jnp.ones((2, 4, 5, 7), dtype=jnp.float32)
    provider = odml.OdmlConversionProvider(
        rules, {'kernel': lifted_static_rhs}, {}
    )
    aux_data.set(rhs, odml_ops.AuxDataKey.WEIGHT_NAME, 'kernel')
    aux_data.set(rhs, odml_ops.AuxDataKey.IS_ORIGINAL_WEIGHT, True)
    calls = []
  ...
  def test_linen_einsum_single_axis_scale_is_not_flattened(self):
    """Tests an already supported scale vector stays on the normal path."""
    rules = [qconfig.QuantizationRule(module_path='.*', weight_qtype=jnp.int8)]
    lhs = jnp.ones((2, 3, 5), dtype=jnp.float32)
    rhs = jnp.ones((5, 7), dtype=jnp.float32)
    provider = odml.OdmlConversionProvider(rules, {'kernel': rhs}, {})
    aux_data.set(rhs, odml_ops.AuxDataKey.WEIGHT_NAME, 'kernel')
    aux_data.set(rhs, odml_ops.AuxDataKey.IS_ORIGINAL_WEIGHT, True)
    calls = []
  ...
  def test_nnx_einsum_multi_axis_weight_conversion(self):
    """Tests RHS scale flattening through the NNX parameter path."""
    rules = [
        qconfig.QuantizationRule(
            module_path='.*',
            weight_qtype=jnp.int8,
