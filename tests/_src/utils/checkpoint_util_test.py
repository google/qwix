...
class RestoreQuantizationRulesTest(parameterized.TestCase):
  ...
  def test_restore_rules_wildcards(self):
    checkpoint_params = {
        "layers": {
            0: {
                "kernel": {
  ...
  def test_restore_rules_no_act(self):
    ...
    rules = checkpoint_util.restore_quantization_rules(
        checkpoint_params, tile_size=4, act_qtype=None
    )
    expected_rules = [
        qconfig.QuantizationRule(
            module_path="",
  ...
  def test_restore_rules_custom_act(self):
    ...
    rules = checkpoint_util.restore_quantization_rules(
        checkpoint_params, tile_size=4, act_qtype=jnp.uint8
    )
    expected_rules = [
        qconfig.QuantizationRule(
            module_path="",
