...
class MultiPassDotTest(parameterized.TestCase):
  ...
  def test_transformer_attention_multipass_accuracy(self):
    ...
    self.assertGreater(snr_tri - snr_1p, 17.0 if is_tpu() else 20.0)
  ...
  def test_normal_distribution_sqnr_benchmark(self):
    ...
    err_tri_thresh = 0.005 if is_tpu() else 0.002
    tri_bf16_thresh = 47.5 if is_tpu() else 49.0
    self.assertGreater(snr_fp8_tri, tri_thresh)
    self.assertGreater(snr_fp8_tri_bf16, tri_bf16_thresh)
    self.assertLess(err_fp8_tri, err_tri_thresh)
    self.assertLess(err_fp8_tri_bf16, 0.006 if is_tpu() else 0.005)

    # 4-pass Full Cross FP8: > 55.0 dB in float32
    full_thresh = 50.0 if is_tpu() else 55.0
    err_full_thresh = 0.005 if is_tpu() else 0.002
    self.assertGreater(snr_fp8_full, full_thresh)
    self.assertLess(err_fp8_full, err_full_thresh)
