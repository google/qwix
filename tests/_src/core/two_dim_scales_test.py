...
class TwoDimScalesTest(parameterized.TestCase):
  ...
  def test_dot_general_qt_2d_scales_backward(
    ...
    # Compute the loss and gradients for the quantized dot_general.
    def loss_fn_qt(x, y):
      return dot_general_qt.dot_general_qt(x, y, dn, config).mean()

    qt_grads = jax.grad(loss_fn_qt, argnums=(0, 1))(lhs_array, rhs_array)

    # Compute the intermediate primals of the forward pass for comparison.
    intermediate_primals = dot_general_qt.dot_general_qt(
        lhs_array, rhs_array, dn, config
    )

    def final_loss_fn(x):
      return x.mean()

    _, final_loss_vjp_fn = jax.vjp(final_loss_fn, intermediate_primals)

    # Calculate the intermediate gradient for comparison.
    intermediate_grad = final_loss_vjp_fn(1.0)[0]

    # Quantize the lhs and rhs residuals.
    lhs_how = dot_general.get_how_to_quantize(
        dimension_numbers=dn,
        ndims=(2, 2),
        for_lhs=True,
    ...
    # Quantize the gradients if necessary.
    g_how_kwargs = {
        'dimension_numbers': dn,
        'ndims': (2, 2),
        'tile_size': tile_size,
    }
    if dlhs_grad_qtype is not None:
      lhs_g_how = dot_general.get_how_to_quantize(
          **g_how_kwargs,
          for_lhs=True,
          qtype=config.dlhs_grad_qtype,
      )
      lhs_grad_q = qarray.quantize(intermediate_grad, lhs_g_how)
    else:
      lhs_grad_q = intermediate_grad

    if drhs_grad_qtype is not None:
      rhs_g_how = dot_general.get_how_to_quantize(
          **g_how_kwargs,
          for_lhs=False,
          qtype=config.drhs_grad_qtype,
      )
      rhs_grad_q = qarray.quantize(intermediate_grad, rhs_g_how)
    else:
      rhs_grad_q = intermediate_grad

    dlhs = dot_general.dot_general(lhs_grad_q, rhs_q.T, dn)
    drhs = dot_general.dot_general(lhs_q.T, rhs_grad_q, dn)
