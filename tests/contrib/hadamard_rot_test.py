...
class HadamardRotTest(parameterized.TestCase):
  ...
  def test_create_hadamard_matrix(self, power, row_sign_flip, col_sign_flip):
    """Tests that HH^T = dI."""
    key = jax.random.key(0)
    had = hadamard_rot._create_hadamard_matrix(
  ...
  def test_apply_hadamard_lhs(self, idx):
    ...
    lhs_rotated = hadamard_rot._apply_hadamard_lhs(lhs, had, idx)
  ...
  def test_apply_hadamard_rhs(self, idx):
    ...
    rhs_rotated = hadamard_rot._apply_hadamard_rhs(rhs, had, idx)
  ...
  def test_hadamard_rotate_multiply_different_axes(
      self, idx, row_sign_flip, col_sign_flip
  ):
  ...
  def test_hadmard_matrix_construction_errors(self):
    """Tests that the Hadamard matrix construction raises errors as expected."""
    # negative power
    with self.assertRaises(ValueError):
      hadamard_rot._create_hadamard_matrix(
          -1,
          None,
          row_sign_flip=False,
          col_sign_flip=False,
  ...
  def test_error_on_multiple_reduction_axes(self):
    """Tests that multiple reduction axes raise errors."""
